...
<div class="rs">
  ...
  <p class="name-stud">{{$student->surname." ".mb_substr($student->name, 0, 1).".".mb_substr($student->patronymic, 0, 1)."."}}</p>
  <div class="body">
    <div class="lentmark" data-toggle="tooltip" data-html="true" title="Предварительная оценка">
      <span class="mark">{{CalculateController::getmarkStudent($rs->id, $id_user)}}</span>
    </div>
    <div class="lentatt" data-toggle="tooltip" data-html="true" title="Аттестация на данный момент">
      <span class="att">{{CalculateController::getAttMark($rs->id, $id_user)}}</span>
    </div>
    <div class="progress-stud">
      <p class="title-progree">Успеваемость:</p>

      <p><i class="fas fa-walking"></i> Посещаемость</p>
